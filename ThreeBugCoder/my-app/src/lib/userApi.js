...
export async function fetchSearchEverything(query, { signal } = {}) {
  ...
  return requestUser(`/recipes/search/all?${params.toString()}`, {
    signal,
    withAuth: false,
  });
}
...
export async function fetchUserDashboard({ signal } = {}) {
  ...
  const products = extractCollection(productsResponse, ["products", "items", "list", "data"]);
  const categories = extractCollection(categoriesResponse, ["categories", "category", "items", "list", "data"]);
