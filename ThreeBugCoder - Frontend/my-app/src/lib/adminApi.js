...
export async function fetchRecipes({ search = "", skip = 0, limit = 100, signal } = {}) {
  const params = new URLSearchParams();
  if (search) params.set("search", search);
  params.set("skip", String(skip));
  params.set("limit", String(limit));

  return requestAdmin(`/recipes/?${params.toString()}`, { signal });
}

export async function createRecipe(payload, { signal } = {}) {
  return requestAdmin("/recipes/", {
    method: "POST",
    body: payload,
    signal,
  });
...
export async function readRecipe(recipeId, { signal } = {}) {
  return requestAdmin(`/recipes/${encodeURIComponent(recipeId)}`, { signal });
}

export async function updateRecipe(recipeId, payload, { signal } = {}) {
  return requestAdmin(`/recipes/${encodeURIComponent(recipeId)}`, {
    method: "PUT",
    body: payload,
    signal,
  });
